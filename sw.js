...
self.addEventListener('fetch',function(e){
  ...
  if(url.indexOf('supabase.co')>-1||url.indexOf('cdn.jsdelivr.net')>-1)return; // veri/CDN her zaman ağdan
  if(e.request.method!=='GET')return;
  e.respondWith(
    fetch(e.request).then(function(res){
      if(res&&res.status===200&&url.indexOf(self.location.origin)===0){var cl=res.clone();caches.open(CACHE).then(function(c){c.put(e.request,cl);});}
      return res;
